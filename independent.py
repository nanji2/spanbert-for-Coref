...
class CorefModel(object):
  def __init__(self, config):
    ...
    # config["genres"]: ["bc", "bn", "mz", "nw", "pt", "tc", "wb"]
  ...
  def get_predictions_and_loss(self, input_ids, input_mask, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map):
    ...
    mention_doc0 = model.get_sequence_output()

    self.dropout = self.get_dropout(self.config["dropout_rate"], is_training)

    num_sentences = tf.shape(mention_doc0)[0]
    max_sentence_length = tf.shape(mention_doc0)[1]
    mention_doc = self.flatten_emb_by_sentence(mention_doc0, input_mask)
    ...
    return [candidate_starts, candidate_ends, candidate_mention_scores, top_span_starts, top_span_ends, top_antecedents, top_antecedent_scores,mention_doc], loss
